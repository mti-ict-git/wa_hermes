...
- Phase 10 hardening is complete for the current scope, but the service still uses in-process state for Hermes continuity and webhook deduplication. The next durability step remains SQLite, not Redis.
- The next architecture step is no longer "model talks to backend directly". The trusted pattern is:
  - OpenWA metadata -> local role resolution -> signed `AuthContext`
  - Marisa -> typed intent
  - local validator -> approved read-only adapter
...
- WhatsApp helpdesk design decisions are now captured in `docs/helpdesk-whatsapp-design.md`, but these policy questions remain open:
  ...
  - which read-only AD and Veeam intents should enter the initial typed-intent allowlist
  - what TTL and replay strategy should be final for signed `AuthContext`
- Phase 5 live OpenWA verification showed incoming private chats may arrive with `chatId` / `from` in `@lid` form instead of phone-like `@c.us`. Group mention detection now supports configured LID-style bot aliases, but private sender identity still needs a confirmed strategy to map `@lid` senders to canonical AD-backed phone identity.
